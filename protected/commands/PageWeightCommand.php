    ...
    public function actionParse($url) {
    	...
    	do{
    		...
    		if($page){
    			...
    			$pageUrl=$this->_parseUrl['scheme'].'://'.$this->_parseUrl['host'].'/';
    		}
	    	...
	    	if( $response['body'] ){
		    	...
		    	foreach($html->find('a') as $element) { 
		    		...
		    		if(strpos($url, 'function')){
						file_put_contents(__DIR__ . '/../runtime/pageWeight/'.$url, $response['body']);
					}
		    		...
		    		if($page){
			    		...
			    		$pageLink->uri_in = $uri;
			    		$pageLink->anchor = $element->innertext;
						$pageLink->save();
		    		}
